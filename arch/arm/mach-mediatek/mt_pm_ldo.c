...
#ifdef VENDOR_EDIT
/*Zhangkw@CameraDrv.camera, 2013/02/25, add new interface needed by camera*/
bool hwPowerOnPre(MT65XX_POWER powerId, MT65XX_POWER_VOLTAGE powerVolt, char *mode_name)
{
    UINT32 i = 0;
    int j=0, k=0;
...
EXPORT_SYMBOL(hwPowerOnPre);

bool hwPowerOnRealEnable(MT65XX_POWER powerId)
{
          pmic_ldo_enable(powerId, KAL_TRUE);
          return TRUE;
}
EXPORT_SYMBOL(hwPowerOnRealEnable);
...
#ifdef VENDOR_EDIT
/*Zhangkw@CameraDrv.camera, 2013/02/25, add new interface needed by camera*/
bool hwPowerDownPre(MT65XX_POWER powerId, char *mode_name)
{
    UINT32 i;
#if 1	
    BOOL bFind = FALSE;    
...
#endif	
    //pmic_ldo_enable(powerId, KAL_FALSE); //zhangkw del

    return TRUE;    
}
EXPORT_SYMBOL(hwPowerDownPre);

bool hwPowerDownRealDisable(MT65XX_POWER powerId)
{
       pmic_ldo_enable(powerId, KAL_FALSE);
	return TRUE;
}
EXPORT_SYMBOL(hwPowerDownRealDisable);
#endif /*VENDOR_EDIT*/
